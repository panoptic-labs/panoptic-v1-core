...
Panoptic is a permissionless options trading protocol. It enables the trading of perpetual options on top of any [Uniswap V4](https://uniswap.org/) pool
...
├── SemiFungiblePositionManager - "The 'engine' of Panoptic - manages all Uniswap V4 positions in the protocol as well as being a more advanced, gas-efficient alternative to NFPM for Uniswap LPs"
├── base
...
├── interfaces
│   └── IV3CompatibleOracle - "Interface defining the oracle contract used by Panoptic, which may be a Uniswap V3 pool or custom implementation"
├── types
...
└── libraries
    ...
    └── V4StateReader - "Library to retrieve state information from Uniswap V4 pools via `extsload`"
```
...
Panoptic can be deployed on any chain with a Uniswap V4 instance. To go through with the deployment, several environment variables need to be set:

- `DEPLOYER_PRIVATE_KEY` The private key of the EOA deploying the contracts
- `POOL_MANAGER` The address of the canonical Uniswap V4 Pool Manager on the chain
